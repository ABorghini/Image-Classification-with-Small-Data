...
class LearningMethod(ABC):
    ...
    **hparams
    ...
    def evaluate_epoch(self, model, loader, criterion, show_progress=True):
        ...
        return (
            ClassificationMetrics(total_loss / num_samples, total_acc / num_samples),
    ...
    def train_model(
        ...
        for ep in progbar:
            ...
            train_metrics = self.train_epoch(
                model, train_loader, optimizer, criterion, **train_args
            )

            if not (
                isinstance(train_metrics, dict)
                or isinstance(train_metrics, OrderedDict)
            ):
            ...
            if (
                ...
                and (ep == 0 or (ep + 1) % eval_interval == 0)
            ):

                val_metrics, curr_acc = self.evaluate_epoch(
                    model, val_loader, criterion, **eval_args
                )
                ...
                if best_acc < curr_acc:
                    ...
                    torch.save(
                        model.state_dict(),
                        "/content/drive/MyDrive/Homework2/checkpoints/checkpoint_"
                        + str(ep)
                        + ".pth",
                    )
                    best_acc = curr_acc
                ...
                if not (
                    ...
                    or isinstance(val_metrics, OrderedDict)
                ):
                ...
                print(
                    metrics["val_loss"][-1],
                    np.mean(metrics["val_loss"][-5:]),
                    metrics["val_accuracy"][-1],
                    np.mean(metrics["val_accuracy"][-5:]),
                )
            if "lr" not in metrics:
                metrics["lr"] = []
            metrics["lr"].append(optimizer.param_groups[0]["lr"])
            progbar.set_postfix(
                OrderedDict((key, values[-1]) for key, values in metrics.items())
            )
    ...
    def train(
        ...
        model = self.create_model(
            architecture, train_data.num_classes, train_data.num_input_channels
        ).cuda()
        ...
        optimizer, scheduler = self.get_optimizer(
            par_model, max_epochs=epochs, max_iter=iterations
        )
    ...
    def evaluate(
        ...
        pred = predict_class_scores(model, test_data, batch_size=batch_size).argmax(
            axis=-1
        )
    ...
    def create_model(
        self, arch: str, num_classes: int, input_channels: int = 3
    ) -> nn.Module:
        ...
        return build_classifier(
            arch=arch, num_classes=num_classes, input_channels=input_channels
        )
    ...
    def __repr__(self):
        ...
        return (
            ...
            + ", ".join(
                ...
                for param_name, param_val in self.hparams.items()
            )
            + ")"
        )
            ...
            if self.hparams["rand_shift"] > 0:
                ...
                transforms.append(
                    tf.RandomResizedCrop(
                        ...
                        scale=(self.hparams["min_scale"], self.hparams["max_scale"]),
                    )
                )
